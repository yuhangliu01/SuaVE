...
def simulate_random_dag(d: int,
    ...
    B_perm = B #P.T.dot(B).dot(P)
    U = np.random.uniform(low=w_range[0], high=w_range[1], size=[d, d, n_seg])
    W = U
    #G = U
    for seg in range(n_seg):
        U[np.random.rand(d, d) < 0.5, seg] *= -1
        W[:, :, seg] = (B_perm != 0).astype(np.float32) * U[:,:,seg]
        #G[:, :, seg] = nx.DiGraph(W[:, :, seg])
    G = nx.DiGraph(W[:, :, 0])
    return W, G # 
...
def to_one_hot(x, m=None):
    ...
    for i, xi in enumerate(x):
        ...
        xoh[i][np.arange(xi.size), xi.astype(np.int_)] = 1
    return xoh
...
def generate_nonstationary_sources(n_per_seg: int, n_seg: int, d: int, prior='lap', var_bounds=np.array([0.5, 3]),
                                   dtype=np.float32, uncentered=True):
    """
    ...
    if uncentered:
        m = np.random.uniform(-2, 2, (n_seg, d))
    else:
        m = np.zeros((n_seg, d))
...
def gen_TCL_data_ortho(n_per_seg, n_seg, d_sources, d_data=None, n_layers=3, prior='lap', activation='lrelu', batch_size=250,
                  seed=10, slope=.2, var_bounds=np.array([0.01, 3]), lin_type='uniform', n_iter_4_cond=1e4,
                  dtype=np.float32, uncentered=False, noisy=0, relation=None, gdegree=None, gtype=None):
    """
    ...
    assert activation in ['lrelu', 'sigmoid']

    # generate non-stationary data:
    Nobs = n_per_seg * n_seg  # total number of observations
    ...
    if prior == 'Lap':
        dat = np.random.laplace(0, 1, (Nobs, d_sources))
        dat = scale(dat)  # set to zero mean and unit variance
    elif prior == 'gauss':
        dat = np.random.normal(0, 1, (Nobs, d_sources))
        dat = scale(dat)
    else:
    ...
    modMat = np.random.uniform(0.1, 2, (d_sources, n_seg))
    # meanMat = np.random.uniform(0, 5, (Ncomp, Nsegment))
    if uncentered:
        meanMat = np.random.uniform(-2, 2, (d_sources, n_seg))
    else:
        meanMat = np.zeros((d_sources, n_seg))
    
    
    # meanMat = np.random.uniform(0.5, 2, (Ncomp, Nsegment))
    # randsign = np.random.randint(0, 2, (Ncomp, Nsegment))
    # index = np.where(randsign == 0)
    # meanMat[index] = meanMat[index]*-1
    # # now we adjust the variance within each segment in a non-stationary manner

    # sig2 = np.ones((Nobs, 1))
    # sig3 = np.ones((Nobs, 1))
    # weightMat = np.random.uniform(-1, 1, (Ncomp, Nsegment))
    
    #weightMat[1:2,15:] = weightMat[1:2,15]
    #weightMat[2:3,0:15] = weightMat[2:3,0]
    ...
    for seg in range(n_seg):
        ...
        #sig2[segID, :] = np.multiply(sig2[segID, :], weightMat[1:2, seg]) # weights of signal2
        #sig3[segID, :] = np.multiply(sig3[segID, :], weightMat[2:3, seg]) # weights of signal3


    causalDATA = np.copy(dat)
    if relation:
        W, G = simulate_random_dag(d_sources, gdegree, gtype, n_seg)
        X = np.zeros([np.size(dat, 0), np.size(dat, 1)])
        XX = np.zeros([n_per_seg, np.size(dat, 1)])
        for seg in range(n_seg):
            segID = range(n_per_seg * seg, n_per_seg * (seg + 1))
            W_seg = W[:,:,seg] #nx.to_numpy_array(G)
            d = W_seg.shape[0]
            ...
            for j in ordered_vertices:
                ...
                if 'linear' in relation:
                    #XX = X[segID, parents]
                    eta = XX[:, parents].dot(W_seg[parents, j])
                if 'Gaussian' in relation:
                    XX[:, j] = eta + dat[segID, j]
            X[segID, :] = XX
            causalDATA = X

    # now we are ready to apply the non-linear mixtures:
    #a, _ = mcc(dat, causalDATA, permute=False)

    mixedDat = np.copy(causalDATA)
    ...
    for l in range(n_layers - 1):
        ...
        A = ortho_group.rvs(d_sources).astype(causalDATA.dtype)  # generateUniformMat( Ncomp, condThresh )
        mixingList.append(A)

        # we first apply non-linear function, then causal matrix!
        if activation == 'lrelu':
            mixedDat = leaky_ReLU(mixedDat, slope)
    ...
    if not batch_size:
        return [causalDATA.astype(dtype)], [mixedDat.astype(dtype)], to_one_hot([labels.astype(dtype)], m=n_seg), meanMat.T.astype(dtype), modMat.T.astype(dtype), W, nx.to_numpy_array(G), [dat]
    else:
        idx = np.random.permutation(Nobs)
        Xb, Sb, Ub = [], [], []
        nb = []
        n_batches = int(Nobs / batch_size)
        for c in range(n_batches):
            Sb += [causalDATA[idx][c * batch_size:(c + 1) * batch_size]]
            Xb += [mixedDat[idx][c * batch_size:(c + 1) * batch_size]]
            Ub += [labels[idx][c * batch_size:(c + 1) * batch_size]]
            nb += [dat[idx][c * batch_size:(c + 1) * batch_size]]
        return Sb, Xb, to_one_hot(Ub, m=n_seg), meanMat.T.astype(dtype), modMat.T.astype(dtype), W, nx.to_numpy_array(G), nb



    #return [causalDATA], [mixedDat], to_one_hot([labels], m=n_seg), meanMat, modMat #{'source': causalDATA, 'obs': mixedDat, 'labels': labels, 'mixing': mixingList, 'noise': dat}
...
def generate_data(n_per_seg, n_seg, d_sources, d_data=None, n_layers=3, prior='lap', activation='lrelu', batch_size=250,
    ...
    for l in range(n_layers - 1):
        # generate causal matrix first:
        A = generate_mixing_matrix(d_sources, d_data, lin_type=lin_type, n_iter_4_cond=n_iter_4_cond, dtype=dtype)
        #A = ortho_group.rvs(d_sources).astype(sources.dtype)  # generateUniformMat( Ncomp, condThresh )
        mixingList.append(A)
        # we first apply non-linear function, then causal matrix!
        X = act_f(X)
        # apply mixing:
...
def save_data(path, *args, **kwargs):
    ...
    Sb, Xb, Ub, m, L, W, G, n = gen_TCL_data_ortho(*args, **kwargs) # generate_data(*args, **kwargs) #gen_TCL_data_ortho(*args, **kwargs) # generate_data(*args, **kwargs)
    ...
    np.savez_compressed(path, s=Sb, x=Xb, u=Ub, m=m, L=L, w=W, g = G, n = nb)
    print(' ... done')
...
class DataLoaderGPU:
    ...
    def __init__(self, path, batch_size, shuffle=True):
        ...
        #s=Sb, x=Xb, u=Ub, m=m, L=L, w=W, g = G, n = nb
        self.s = torch.from_numpy(data['s']).to(self.device)
        self.x = torch.from_numpy(data['x']).to(self.device)
        self.u = torch.from_numpy(data['u']).to(self.device)
        #b=torch.randperm(self.u.size(0))
        #self.fakeu = self.u[b]
        self.nb = torch.from_numpy(data['n']).to(self.device)
        
        self.graph_w = torch.from_numpy(data['w']).to(self.device)
        self.graph_g = data['g']
    ...
    def __iter__(self):
        for b in range(self.len):
            ...
            yield self.x[idx], self.u[idx], self.s[idx], self.nb[idx]
...
if __name__ == '__main__':
    ...
    parser.add_argument('-c', '--causal', default='linear_Gaussian', dest='c',
                        help='causal relation - default: `linear`')
    parser.add_argument('-dg', '--degree', default=2, dest='dg',
                        help='the number of degree in generated DAG graph - default: 0')
    parser.add_argument('--graph-type', type=str, default='ER',
                    help='the type of DAG graph by generation method')
    

    parser.add_argument('-a', '--activation', default='lrelu', dest='a', #`lrelu`, `xtanh` or `sigmoid`
                        help='activation function of the generating MLP - default: `xtanh`')
    parser.add_argument('-u', '--uncentered', action='store_true', default=True,
    ...
    root = '/data/Yuhang/dagVAE-master/data/'
    ...
    if not os.path.exists(path_to_dataset) or args.s is None:
        kwargs = {"n_per_seg": args.nps, "n_seg": args.ns, "d_sources": args.dl, "d_data": args.dd, "n_layers": args.nl,
                  ...
                  "noisy": args.noisy, "relation": args.c, "gdegree": args.dg, "gtype": args.graph_type}
        save_data(path_to_dataset, **kwargs)
