...
def mean_corr_coef_np(x, y, method='pearson', permute=True):
    """
    ...
    cc = np.abs(cc)
    #cc[np.isnan(cc)]=0
    A = np.arange(np.size(cc,0))
    if permute:
        score = cc[linear_sum_assignment(-1 * cc)].mean()
        _, A = linear_sum_assignment(-1 * cc)
    else:
        score = np.diagonal(cc).mean()
    return score, A #score


def mean_corr_coef(x, y, method='pearson', permute=True):
    if type(x) != type(y):
        raise ValueError('inputs are of different types: ({}, {})'.format(type(x), type(y)))
    if isinstance(x, np.ndarray):
        return mean_corr_coef_np(x, y, method, permute)
